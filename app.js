...
$.get(
  ...
  function buildTable(data) {
    for (let i = 0; i < data.length; i++) {
      ...
      var $tableRow = $(
        `<tr>  <td class ="data-rank">${$rank}</td> <td class="data-name">${$name}</td> <td class="data-symbol">${$symbol}</td> <td class="data-price">${new Intl.NumberFormat(
          "en-GB",
          { style: "currency", currency: "GBP" }
        ).format(
          $price
        )}</td> <td class="data-twentyFourHours">${$twentyFourHours.toLocaleString()}</td> <td class="data-volume">${new Intl.NumberFormat(
          "en-GB",
          { style: "currency", currency: "GBP" }
        ).format(
          $volume
        )}</td> <td class="data-marketCap">${new Intl.NumberFormat("en-GB", {
          style: "currency",
          currency: "GBP",
  ...
  function trending(data) {
    for (let key in data) {
      for (let i = 0; i < data[key].length; i++) {
        ...
        var $rank = data[key][i].item.market_cap_rank;
        var $img = data[key][i].item.small;
        var $name = data[key][i].item.name;
        var $symbol = data[key][i].item.symbol;
        ...
        if ($rank == null) {
          ...
          let $card = $(
            `<div class="trending-card"><span class="trending-rank">${$rank}</span> ${$completeName} <span class="trending-symbol">${$symbol}</span></div>`
          );
